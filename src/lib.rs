...
pub fn central_divide_match(mut n: u64, mut m: u64, p: u64, m_inv: u64) -> bool {
    ...
    // Here we go through the digits in base p of n and m.
    // We check if for either one of them one of those digits is bigger than
    // `p/2`. We do this until we are done with both numbers `(n > 0 || m > 0`).
    // There are two cases when we ca` exit early:
    // - `ndiv && mdiv`: we already have seen digits > p/2 for both `n` and `m`.
    // - `n == m && ndiv == mdiv`: here the remaining digits will be the same and
    //    both if we have seen digits > p/2 is either true for both `n` and `m` 
    //    or false for both `n` and `m`.
